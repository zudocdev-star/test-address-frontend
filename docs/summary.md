...
**Purpose:** Powers the **State dropdown** and the **District dropdown**. Parsed first to build the base hierarchy.
...
**Purpose:** Provides **local body entities** (municipalities, panchayats, town councils, etc.) that appear in the location search. Joined with `state_district_data.json` on the `state|district` composite key to ensure only valid districts are included.
...
**Purpose:** Provides **village and subdistrict entities** for the location search. Due to its large size (~92 MB), it is loaded **asynchronously in the background** after the first two datasets complete, and parsed in chunks to avoid freezing the UI.
...
2. **Phase 2 (Sequential):** After Phase 1 completes, `villages_data.json` is fetched and parsed in chunks of 8,000 lines per iteration, yielding to the event loop (`setTimeout(resolve, 0)`) between chunks to keep the UI responsive.
...
| `buildLocationDistrictKeys()` | `Loc1Record[]` | `Set<string>` | Creates `"state\|district"` keys for join validation |
| `buildJoinedLocationRows()` | `Loc1Record[]`, `FinalMappingRecord[]` | `LocationRecord[]` | Inner-joins local bodies with valid districts |
| `buildIndiaLocationsIndex()` | Raw NDJSON string | `Map<string, LocationRecord[]>` | Indexes villages + subdistricts by `"state\|district"` key |
...
1. **Builds a pool** of searchable records by combining:
   - **Local bodies** from `locationRows` (filtered to selected state + district)
   - **Villages + subdistricts** from `indiaLocationRows` (pre-filtered by `App` via the index map)
2. **Filters** the pool with a case-insensitive substring search across `entityName`, `subdistrictNameEnglish`, `localBodyNameEnglish`, `localBodyTypeName`, and `entityType`.
...
3.  App component mounts
    ...
    │   ├── fetch(localBodies_data.json)      ─┤  Parallel
    │   │                                      │
    ...
    │   └── isLoading=false
    │
    ├── useEffect #2 → loadIndiaLocations() (fires after useEffect #1 completes)
    │   ├── fetch(villages_data.json)   // ~92 MB
    │   ├── buildIndiaLocationsIndex()  // chunked parse, 8000 lines/chunk
    │   │   ├── Parse chunk → yield to event loop → parse next chunk
...
4.  User interaction — Form step
    ...
    │   └── LocationSearchRow filters pool → shows up to 35 suggestions
...
### `App` ([App.tsx](file:///c:/Users/Lenovo/OneDrive/Desktop/New%20folder/vite-project/src/App.tsx))

The root component. Responsibilities:
- **Data loading & error handling** — Manages fetch lifecycle for all 3 datasets with loading/error states
...
### `LocationSelect` ([LocationSelect.tsx](file:///c:/Users/Lenovo/OneDrive/Desktop/New%20folder/vite-project/src/components/LocationSelect.tsx))

The main location picker fieldset. Responsibilities:
- **State dropdown** (`SelectRow`) — Populated from `CountryData[].states`
- **District dropdown** (`SelectRow`) — Filtered by selected state
- **Location search** (`LocationSearchRow`) — Type-ahead over combined local bodies + villages + subdistricts
- **Cascade reset** — If state changes, district and entity are cleared. If district changes, entity is cleared.
...
- **Filtering** — Case-insensitive substring matching across multiple fields
- **Keyboard navigation** — Arrow keys, Enter to select, Escape to close
- **Selection chips** — Shows entity type, subdistrict, and local body details after selection
...
3. **District-keyed index:** Villages and subdistricts are indexed by a composite `"state|district"` key. When the user selects a district, only the relevant slice is extracted from the index — avoiding a full scan of 92 MB of data on every keystroke.

4. **Join validation:** Local body records from `localBodies_data.json` are inner-joined against valid `state|district` pairs from `state_district_data.json`, ensuring only legitimate district combinations appear.

5. **Country fixed to India:** The UI hardcodes the country as India. The `CountryData` type supports multiple countries structurally, but only India is populated.

6. **React Compiler enabled:** The project uses `babel-plugin-react-compiler` for automatic memoization and optimization, reducing the need for manual `React.memo`, `useMemo`, and `useCallback` in some cases.

7. **Max 35 suggestions:** The search caps results at 35 to avoid rendering performance issues with long lists of unvirtualized DOM elements.

---

*Generated on 2026-06-03. Refer to the source code for the most up-to-date implementation details.*
